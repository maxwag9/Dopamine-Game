...
class Ball:
    ...
    def move(self):
        ...
        if ball_pos.x < 0 or ball_pos.x > screen_width:
            ...
            self.body.position = (max(0, int(min(ball_pos.x, screen_width))), ball_pos.y)  # Clamp position inside bounds

        if ball_pos.y < 0 or ball_pos.y > screen_height:
            self.body.velocity = (ball_vel.x, -ball_vel.y)  # Reverse Y velocity
            self.body.position = (ball_pos.x, max(0, int(min(ball_pos.y, screen_height))))  # Clamp position inside bounds
...
class Enemy:
    ...
    def move(self):
        for i in range(len(self.red_nemesis_list)-1, -1, -1):  # Iterate in reverse
            ...
            body = red_nemesis["body"]
            shape = red_nemesis["shape"]
            speed = red_nemesis["speed"]
            # Update position based on velocity and apply simple movement

            body.position += body.velocity
            red_nemesis["pos_x"], red_nemesis["pos_y"] = body.position
            ...
            body.angle = math.radians(red_nemesis["rotation"])

            vx, vy = body.velocity  # Unpack x and y components of velocity
            if abs(vx) > 20:
                vx = 10 if vx > 0 else -10  # Limit to 10 while preserving direction
            if abs(vy) > 20:
                vy = 10 if vy > 0 else -10  # Limit to 10 while preserving direction
            body.velocity = (vx, vy)
            ...
            if len(self.red_nemesis_list) < 50:
                global collision_mode
                if collision_mode == 0: collision_mode = 1
                else: collision_mode = 0
                self.wave()

            if red_nemesis["hp"] <= 0:
                space.remove(red_nemesis["body"], red_nemesis["shape"])
                particles.create_particle(red_nemesis["pos_x"], red_nemesis["pos_y"], 30, 3,
                                          red_nemesis["color"], "damage", 2)
    ...
    def choose_random_props(self):
        ...
        for red_nemesis in self.red_nemesis_list:
            if not red_nemesis["random_props_chosen"]:
                ...
                body.position = (red_nemesis["pos_x"], red_nemesis["pos_y"])
                body.velocity = (red_nemesis["vel_x"], red_nemesis["vel_y"])
                # Create a rectangular shape for the enemy
                shape = pymunk.Poly.create_box(body, (red_nemesis["size"], red_nemesis["size"]))
                shape.elasticity = 1
                shape.friction = 0
                space.add(body, shape)
                red_nemesis["body"] = body
    ...
    def draw_menu(self):
        ...
        for button_props in self.button_list:
            ...
            if button_props["button_label"] == "Ball BL":
                button_props["pos_x"] = balls[0].body.position[0] - width / 2
                button_props["pos_y"] = balls[0].body.position[1] - height / 2 + 100
...
while True:
    ...
    if time_since_last_frame >= (1000 // fps):  # Run FPS updates
        ...
        space.step(time_since_last_frame/1000)
